...
dPar  = {
            'a_Mc'         : param_value['Mc'].values, # , #3.0,4.0]),
...
for f_Mc in dPar['a_Mc']:
    ...
    # Check to change name
    clust_file = file_in.replace( '.mat', 'Mc_%.1f_clusters.mat'%( f_Mc))
    dClust = data_utils.loadmat( os.path.join( dir_in,clust_file), )
    ...
    #                    save to ASCII text
    #============================================================================================================
    file_out = '%s/%s_Nas_MS_Mc_%.1f.txt'%(dir_in, file_in.split('.')[0], f_Mc)#, dPar['magRound'])
...
import pickle
fileout = '%s/%s_cluster_dic_%.1f.pickle'%(dir_in, file_in.split('.')[0], f_Mc)
# Store data (serialize)
with open(fileout, 'wb') as handle:
    pickle.dump(dClust, handle, protocol=pickle.HIGHEST_PROTOCOL)
