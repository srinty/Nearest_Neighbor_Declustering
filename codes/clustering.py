...
def NND_eta( eqCat, dConst, verbose = False, **kwargs):
    ...
    kwargs      - rmax (default: = 500) - maximum space window (for faster computation)
                ...
                - Hypocentral = True, 3d distance computation
    Returns
    -------
    ...
    for jC in range( eqCat.size()):
        ...
        if sel_tau_par.sum() > 0:
            ...
            if 'X' in eqCat.data.keys() and 'Y' in eqCat.data.keys():
                ...
                #  haversine distance
                vR_h = haversine( eqCat.data['Lon'][jC], eqCat.data['Lat'][jC],eqCat.data['Lon'][vcurr_ID], eqCat.data['Lat'][vcurr_ID] )
                d_depth = eqCat.data['Depth'][jC] - eqCat.data['Depth'][vcurr_ID]
                vR = np.sqrt(vR_h**2 + d_depth**2)
...
def rescaled_t_r(catChild, catPar, dConst, **kwargs):
    ...
    if 'X' in catChild.data.keys() and 'X' in catPar.data.keys():
        ...
        # a_R = np.sqrt( (haversine(catChild.data['Lon'], catChild.data['Lat'],
        #            catPar.data['Lon'],   catPar.data['Lat']))**2 +
        #             (catChild.data['Depth'] - catPar.data['Depth'])**2
        #                             ) **dConst['D']*vMagCorr
